...
Speako does **not** fetch email or Teams messages itself. This task is the only thing that
writes to the `external_messages` table; Speako only reads from it and does the
chunking/embedding on its own side (`POST /api/communications/index`, or the "Index
communications" button in Settings).
